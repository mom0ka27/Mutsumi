...
从 `main` 渠道更新成功后，服务端会将对应提交哈希写入本地 `.build-info`。后续检查 `main` 时会与 GitHub 最新提交比较；未通过 `main` 渠道更新过的部署会显示“未记录”，并视为可更新。更新状态仅在当前服务进程内保存；服务重启后客户端会通过当前版本或提交哈希与目标版本比较来确认更新是否完成。
