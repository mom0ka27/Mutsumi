...
class _AnimeHomeViewState extends State<AnimeHomeView> {
  ...
  late Future<List<AnimeRead>> _future;

  @override
  void initState() {
    super.initState();
    _future = _animeService.listAnimes();
  }
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return ListView(
              ...
              children: [Center(child: Text('加载 Anime 失败\n${snapshot.error}'))],
            );
          }
          ...
          if (animes.isEmpty) {
            return ListView(
              padding: EdgeInsets.fromLTRB(24, 24, 24, widget.bottomPadding),
              children: const [Center(child: Text('还没有添加 Anime'))],
            );
          }

          return ListView.separated(
            padding: EdgeInsets.fromLTRB(20, 12, 20, widget.bottomPadding),
            itemBuilder: (context, index) => _AnimeCard(anime: animes[index]),
