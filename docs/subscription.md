...
```
每 15 分钟：
  POST /resources { types:['动画'], after: <游标> }   ← 一页一页翻到上游说没有了
  → ~5-10 条新资源
  → 对每条资源，遍历所有订阅的匹配规则
...
`pageSize=2000`，响应里是 `pagination.pageSize: 100` 加 100 条数据。所以「拿回来的比要的少
⇒ 这是最后一页」这个判断是错的 —— 它会在第一页就宣布抓完，剩下的资源全部丢掉。这正是
一次 7 天冷启动窗口（实测 666 条）被旧的「5 页 × 100」上限截掉四分之一的原因。

现在的翻页只在三种情况下停：

1. 上游自己说 `complete`（`payload.complete` 或 `pagination.complete`）；
2. 这一页一条新的都没有 —— 顺带兜住了「上游忽略 `page` 参数、反复给同一页」；
3. 缺的每一集都已经匹配到可用资源（见下）。

「比请求的少」这条判断仍然保留，但只跟 `pagination.pageSize` **实际生效的值**比，且数的是
原始条目数而不是解析成功的条目数 —— 否则一页里有一条脏数据就会被当成最后一页。

### 提前收工：缺集全部匹配到

feed 严格按时间倒序（实测跨页 `createdAt` 单调递减），所以一旦**已播出但本地没有**的每一集
都拿到了一条**通过规则**的资源，再往后翻只会是更老的东西，可以停。判定要求资源
`accepted`，不只是集数对得上，否则会停在规则本来就会丢掉的候选上。

没有明确缺口时（还没开播、或本地已齐）**不启用**提前收工 —— 空集合会被"平凡满足"，第一页就停，
而翻到底恰恰是为了拿到 Bangumi 还没登记或还没标日期的那一集。
...
subscription:
  ...
  page_size: 100            # 每页条数，上游默认值；超范围会被静默降回 100
  max_pages: 200            # 翻页失控保护；正常翻到上游说没有了或缺集全部匹配到为止
